...
class Endpoint final : public AsyncWrap, public Packet::Listener {
 public:
  ...
  struct Options final : public MemoryRetainer {
    ...
    // All connection IDs generated by this endpoint use a single fixed
    // length so short-header packets can be decoded without per-session
    // state. The default preserves the existing 20-byte CID behavior.
    uint8_t cid_length = CID::kMaxLength;

    // An optional application-provided connection ID generator. When unset,
    // the default cryptographically-random generator is used.
    std::shared_ptr<JSCIDGenerator> cid_generator;
  ...
  // Generate a connection ID using this endpoint's fixed-length policy.
  CID GenerateNewConnectionId();
  CID GenerateNewConnectionId(ngtcp2_cid* cid, size_t length);
