...
An optional synchronous callback used to generate connection IDs created by
the endpoint. The callback receives the configured
[`endpointOptions.cidLength`][] and must return an {ArrayBufferView} containing
exactly that number of bytes. The returned bytes are copied immediately after
the callback returns. Returning a different type or length throws an error.

The callback can be invoked when creating client or server sessions, Retry
packets, preferred address transport parameters, and additional connection IDs
during an active session. It must return a new, unique connection ID on every
call. By default, connection IDs are generated using cryptographically secure
random bytes. The random initial destination connection ID selected by a client
for its peer is not generated by this callback.

Custom connection IDs can encode routing information for an external load
balancer. They must retain enough unpredictable data to prevent collisions and
must not expose plaintext user, server, or deployment identifiers. Connection
IDs issued for the same session should not be linkable by observers.

#### `endpointOptions.cidLength`
...
The fixed length, in bytes, of every connection ID generated by the endpoint.
The value must be between `1` and `20`. Values shorter than `4` increase the
risk of collisions and are not recommended.

QUIC short headers do not encode the connection ID length. All endpoints that
share a connection ID namespace, including endpoints used by a preferred
address, must therefore use the same `cidLength` and compatible routing policy.
...
[`endpointOptions.cidLength`]: #endpointoptionscidlength
