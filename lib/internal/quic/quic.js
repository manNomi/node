 ...
 * @property {Function} [cidGenerator] A synchronous connection ID generator
 * @property {number} [cidLength=20] The connection ID length in bytes
...
class QuicEndpoint {
  ...
  #processEndpointOptions(options) {
    ...
    validateInteger(cidLength, 'options.cidLength', 1, 20);

    let validatedCidGenerator;
    if (cidGenerator !== undefined) {
      validateFunction(cidGenerator, 'options.cidGenerator');
      validatedCidGenerator = function validatedCidGenerator(length) {
        const cid = FunctionPrototypeCall(cidGenerator, undefined, length);
        if (!isArrayBufferView(cid)) {
          throw new ERR_INVALID_ARG_TYPE(
            'options.cidGenerator return value', 'ArrayBufferView', cid);
        }
        const byteLength = isDataView(cid) ?
          DataViewPrototypeGetByteLength(cid) :
          TypedArrayPrototypeGetByteLength(cid);
        if (byteLength !== cidLength) {
          throw new ERR_INVALID_ARG_VALUE(
            'options.cidGenerator return value', cid,
            `must contain exactly ${cidLength} bytes`);
        }
        return cid;
      };
    }
    ...
    return {
      ...
      reusePort,
      cc,
      cidGenerator: validatedCidGenerator,
      cidLength,
      resetTokenSecret,
      tokenSecret,
    };
